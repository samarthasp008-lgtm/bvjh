...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text("Welcome to Sammy's spam bot ⚡ type /help to see available commands")
...
🌟 Available commands: 🌟
 ...
 /switch ⏱️ <min> - Set switch interval (2+ min)
...
async def pair_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user_id = update.effective_user.id
    if not is_authorized(user_id):
        await update.message.reply_text("⚠️ You are not authorised to use, dm owner to gain access! @sammy_here01")
        return
    if not context.args:
